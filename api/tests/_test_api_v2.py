...
from api.models import Event2
from api.serializers import EventSerializer


class EventTestCase2(APITestCase):
    def setUp(self):
        event_raw_1 = {
            'date': '2021-10-01',
            'views': '10',
    ...
    def test_create_event(self):
        """Создать событие."""
        self.assertEqual(4, Event2.objects.all().count(),
                         'Начальное количество событий')
        url = reverse('event_v2')
        data = {
            'date': '2020-10-10',
            'views': '20',
            'clicks': '10',
        ...
        self.assertEqual(5, Event2.objects.all().count(),
                         'Неверное количество событий после создания')

        new_event = Event2.objects.all().last()
    ...
    def test_get_ordering_cpm(self):
        """Сортировка по cpm."""
        url = reverse('event_v2')
        events = Event2.objects.all().order_by('cpm')
    ...
    def test_get_ordering_cpc(self):
        """Сортировка по cpc."""
        url = reverse('event_v2')
        events = Event2.objects.all().order_by('cpc')
